...
export class DistanceMatrixService {
  ...
  private your_access_token:string = "eCPTW8BeWajYjUI2QaNn1plUiMAQC1EGsxMSCAnrNfbjU8XYtIJ04Su7YDDEVGqG";
...
private generateUrlDistance(origen: string, destino:string){//origen y destino en formato string de calle numero, ciudad, provincia, pais
      
      return "https://api.distancematrix.ai/maps/api/distancematrix/json?origins=" + 
              origen + 
              "&destinations=" + 
              destino +
              "&key=" + this.your_access_token;
  }
    ...
    public getApiDistanceMatrix(destino:string):Promise<any>{
      ...
      return this.httpClient.get(this.generateUrlDistance(this.addressDeposit, destino)).toPromise();
